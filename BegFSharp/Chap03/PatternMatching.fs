...
let rec luc x =
    match x with
    | x when x <= 0 -> failwith "value must be greating than 0"
    | 1 -> 1
    | 2 -> 3
    | x -> luc (x - 1) + luc (x - 2)
...
// function for convertng a string to a Boolean
...
// The next two examples demonstrate pattern matching over tuples , with two functions
// that implement a Boolean And and Or using pattern matching. Each takes a
